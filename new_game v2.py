...
class Game:
    ...
    def __init__(self, width, height):
        ...
        self.pre_nuke_sound = pygame.mixer.Sound("./sounds/siren.wav")
        self.nuke_sound = pygame.mixer.Sound("./sounds/explosion4.wav")
    ...
    def play_game(self):
        ...
        nuke_button = Image_Button(self.width - 150 - 150 - 150 - 150 - 50, 550 - 50, 100, 100, "./sprites/nuclear_button.png")
        change_button = True

        yes_button = Image_Button(150, self.height // 2 + 50, 150, 150, "./sprites/yes.png")
        no_button = Image_Button(self.width - 150 - 200, self.height // 2 + 50, 150, 150, "./sprites/no.png")
        ...
        self.wave = 1
        self.level_bar = LevelBar((0, 0, 0), (255, 0, 0), self.width // 2 -  600 // 2, 30, 600, 20, self.wave_info[self.wave - 1])
        ...
        while self.state["quit"] is False: # MAIN GAME LOOP
            ...
            if nuke_crosshair.draw_bool is True:
                if moused[0] == 1 and nuke_cooldown == 0:
                    self.player.nuke(pos[0], pos[1])
                    nuke_crosshair.draw_bool = False
                    nuke_flash = (pos[0], pos[1])
                    nuke_cooldown = 600
            ...
            if moused[0] == 1: # if left mouse clicked
                ...
                if nuke_button.isOver(pos):
                    if nuke_cooldown == 0:
                        nuke_crosshair.draw_bool = True
                        nuke_cooldown = 20
                        # self.player.nuke()
                        # nuke_cooldown = 180
            ...
            if spawn_cooldown == 0:
                if current_spawned <= self.wave_info[self.wave - 1]:
                    num_enemies = 2
                    Generator(self, num_enemies)
                    current_spawned += num_enemies

                spawn_cooldown = self.wave_spawn_cd[self.wave - 1]
                if spawn_cooldown is None:
                    spawn_cooldown = 300
            else:
                spawn_cooldown -= 1
            ...
            nuke_button.draw(self.screen)

            if nuke_crosshair.draw_bool is True:
                pos = pygame.mouse.get_pos()
                nuke_crosshair.draw(self.screen, pos[0], pos[1])
            if nuke_cooldown > 0:
                nuke_cooldown -= 1
                if change_button is True:
                    nuke_button.image = pygame.image.load("./sprites/nuclear_button_pressed.png")
                    nuke_button.image = pygame.transform.scale(nuke_button.image, (nuke_button.width, nuke_button.height))
                    change_button = False
            else:
                if change_button is False:
                    nuke_button.image = pygame.image.load("./sprites/nuclear_button.png")
                    nuke_button.image = pygame.transform.scale(nuke_button.image, (nuke_button.width, nuke_button.height))
                    change_button = True
            ...
            self.level_bar.draw(self.screen)
            ...
            score_text = self.font.render(f"SCORE: {str(self.score)}", True, (255, 255, 255))
            score_textRect = score_text.get_rect()
            self.screen.blit(score_text, score_textRect)

            wave_text = self.font.render(f"WAVE:  {str(self.wave)}", True, (255, 255, 255))
            wave_textRect = wave_text.get_rect(x=0, y=50)
            self.screen.blit(wave_text, wave_textRect)

            if self.level_bar.current_enemies <= 0:
                self.wave += 1
                self.level_bar.max_enemies = self.wave_info[self.wave - 1]
                self.level_bar.current_enemies = self.wave_info[self.wave - 1]
...
class Player:
    ...
    def nuke(self, target_x, target_y):
        self.game.missiles.append(Nuke_missile(self.game, target_x, target_y))
    ...
    def check_collision(self):
        for enemy in self.game.enemies:
            if (enemy.x <= self.x + 10 <= enemy.x + enemy.width) \
                ...
                self.game.score += 1
                self.game.level_bar.current_enemies -= 1
                break
...
class Missile2:
    ...
    def draw(self):
        ...
        if len(self.game.enemies) == 0 and self.explode is False:
            self.x += 5
...
class Nuke_missile:
    def __init__(self, game, x, y):
        ...
        self.x -= self.max_size // 2
        self.y -= self.max_size // 2

        self.image = pygame.image.load("./sprites/nuclear_bomb.png")
        self.image = pygame.transform.scale(self.image, (self.max_size, self.max_size))

        self.explode_image = pygame.image.load("./sprites/mushroom_cloud_explosion.png")
        self.explode_image = pygame.transform.scale(self.explode_image, (500, 500))
        self.explode = False

        self.explode_enemy_image = pygame.image.load("./sprites/explosion2.png")
        self.explode_enemy_image = pygame.transform.scale(self.explode_enemy_image, (100, 100))

        self.countdown = 240

        self.game.pre_nuke_sound.play()
    ...
    def draw(self):
        ...
        if self.explode is False:
            ...
            self.game.pre_nuke_sound.stop()
            self.game.nuke_sound.play()
    
    def check_collision(self):
        if self.explode is True:
            for enemy in self.game.enemies:
                if self.x - 750 < enemy.x < self.x +750 and \
                self.y - 750< enemy.y < self.y + 750: 
                    temp = Missile2(self.game, enemy.x, enemy.y)
                    temp.image = temp.explode_image
                    temp.explode = True

                    self.game.missiles.append(temp)
            ...
            if self.x - 750 < self.game.player.x < self.x + 750 and \
                self.y - 750 < self.game.player.y < self.y + 750:
                if self.game.player.current_health > -100:
...
class Generator:
    def __init__(self, game, num_enemies):
...
class Button2:
    """[summary]
        ...
        Args:
            back_colour ([type]): [description]
            text_colour ([type]): [description]
            x ([type]): [description]
            y ([type]): [description]
            width ([type]): [description]
            height ([type]): [description]
...
class LevelBar:
    def __init__(self, back_colour, inner_colour, x, y, width, height, max_enemies):
        self.back_colour = back_colour
        self.inner_colour = inner_colour
        self.x = x
        self.y = y
        ...
        self.current_enemies = max_enemies
    
    def draw(self, screen):
        pygame.draw.rect(screen, self.back_colour, (self.x,self.y,self.width,self.height), 0)

        front_width = self.width * self.current_enemies // self.max_enemies
        pygame.draw.rect(screen, self.inner_colour, (self.x,self.y,front_width,self.height), 0)
...
class Target:
    def __init__(self, width, height):
        ...
        self.image = pygame.image.load("./sprites/crosshair.png")
        self.image = pygame.transform.scale(self.image, (self.width, self.height))
